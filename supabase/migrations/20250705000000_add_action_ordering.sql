...
-- COMMENTED OUT: Column already exists
-- ALTER TABLE actions ADD COLUMN sort_order INTEGER DEFAULT 0;

-- Create index for better performance when sorting (if not exists)
CREATE INDEX IF NOT EXISTS idx_actions_sort_order ON actions(subject_id, sort_order);

-- Update existing actions to have sequential sort_order using CTE (if needed)
-- COMMENTED OUT: Likely already applied
